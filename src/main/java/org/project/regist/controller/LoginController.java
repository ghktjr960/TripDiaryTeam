...
public class LoginController {
	...
	public String loginGet(MemberVo memberVo, HttpServletRequest req, HttpServletResponse resp, HttpSession session)
			throws Exception {
	...
	public String loginPOST(MemberVo memberVo, HttpServletRequest req, HttpServletResponse resp,
			RedirectAttributes rttr, String rememberCookie, Model model) throws Exception {
		...
		if (authInfo != null) {
			...
			if (pwMatch == true) {
				...
				ResignVo resignVo = memberService.resignChk(authInfo);

				if (session.getAttribute("resignchk") != null) {

					// 여기서부터
					Date date = resignVo.getDelDate();
					...
					logger.info("current: " + df.format(cal.getTime()));
					cal.add(Calendar.DATE, +6);
					logger.info("after: " + df.format(cal.getTime()));
					session.setAttribute("resignTime", df.format(cal.getTime()));

					logger.info("이게 바로 너의 탈퇴신청일 : " + simpleDate.format(date));
				}
	...
	public String logout(HttpSession session, HttpServletRequest req, HttpServletResponse resp) throws Exception {
		 ...
		 * session.invalidate();
		 * 
		 * }else { //session.removeAttribute("authInfo");
		 * //session.removeAttribute("test"); session.invalidate();
		 * logger.info("저장된 아이디는 없는 상태임 "); Cookie cookie = new Cookie("myCookieId",
		 * null); cookie.setPath("/"); cookie.setMaxAge(0); resp.addCookie(cookie); }
		 * 
		 */
