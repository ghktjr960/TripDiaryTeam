...
<body>
	...
	<div class="container">
		...
		<form action="/writeUpdate" method="post"
			enctype="multipart/form-data" class="mt-5 mb-5">
			<div class="writeForm mt-5">
				<div class="writeSelect">
					...
					<select name="year" class="form-select">
						<option selected
							value="<c:out value="${fn:substring(board.tripdate,0,4) }" />">${fn:substring(board.tripdate,0,4) }</option>
						...
						<option selected
							value="<c:out value="${fn:substring(board.tripdate,5,7) }" />">${fn:substring(board.tripdate,5,7) }</option>
						<optgroup label=""></optgroup>
						...
						<option selected
							value="<c:out value="${fn:substring(board.tripdate,8,10) }" />">${fn:substring(board.tripdate,8,10) }</option>
						<optgroup label=""></optgroup>
					...
					<div class="form-check form-check-inline mb-5">
						<input class="form-check-input" type="radio" name="placeCheck"
							value="abroad" onclick="disable_select()"
							<c:if test="${board.place eq 'abroad' }">checked </c:if>>
						<label class="form-check-label">해외</label>
					</div>
					<div class="form-check form-check-inline">
						<input class="form-check-input" type="radio" name="placeCheck"
							onclick="disable_select2()"
							<c:if test="${board.place ne 'abroad' }">checked </c:if>>
						<label class="form-check-label">국내</label>
					</div>

					<select name="place" id="select_place" class="form-select">
						<option value="seoul"
							<c:if test="${board.place eq 'seoul' }">selected </c:if>>서울특별시</option>
						<option value="busan"
							<c:if test="${board.place eq 'busan' }">selected </c:if>>부산광역시</option>
						<option value="daegu"
							<c:if test="${board.place eq 'daegu' }">selected </c:if>>대구광역시</option>
						<option value="incheon"
							<c:if test="${board.place eq 'incheon' }">selected </c:if>>인천광역시</option>
						<option value="gwangju"
							<c:if test="${board.place eq 'gwangju' }">selected </c:if>>광주광역시</option>
						<option value="daejeon"
							<c:if test="${board.place eq 'daejeon' }">selected </c:if>>대전광역시</option>
						<option value="ulsan"
							<c:if test="${board.place eq 'ulsan' }">selected </c:if>>울산광역시</option>
						<option value="sejong"
							<c:if test="${board.place eq 'sejong' }">selected </c:if>>세종특별자치시</option>
						<option value="gyeonggi"
							<c:if test="${board.place eq 'gyeonggi' }">selected </c:if>>경기도</option>
						<option value="gangwon"
							<c:if test="${board.place eq 'gangwon' }">selected </c:if>>강원도</option>
						<option value="chungbuk"
							<c:if test="${board.place eq 'chungbuk' }">selected </c:if>>충청북도</option>
						<option value="chungnam"
							<c:if test="${board.place eq 'chungnam' }">selected </c:if>>충청남도</option>
						<option value="jeonbuk"
							<c:if test="${board.place eq 'jeonbuk' }">selected </c:if>>전라북도</option>
						<option value="jeonnam"
							<c:if test="${board.place eq 'jeonnam' }">selected </c:if>>전라남도</option>
						<option value="gyeongbuk"
							<c:if test="${board.place eq 'gyeongbuk' }">selected </c:if>>경상북도</option>
						<option value="gyeongnam"
							<c:if test="${board.place eq 'gyeongnam' }">selected </c:if>>경상남도</option>
						<option value="jeju"
							<c:if test="${board.place eq 'jeju' }">selected </c:if>>제주도특별자치도</option>
					</select>
				</div>
				<h6 class="mt-5" style="color: red;">
					* 사진 재업로드시 기존 사진들은 삭제됩니다.<br>* 내용 변경만 원한다면 사진 재업로드 버튼을 클릭하지
					마세요!
				</h6>
				<label for="thumbnail" class="btn btn-outline-secondary mt-3"
					id="image_container"> <span class="imgText">대표사진
						재업로드 (최대 1장)</span>
				...
				<div class="mt-3" id="image_container">
					<img class="board-thumbnail border border-secondary"
						src="<spring:url value='/thumbnail/${mainImg }'/>">
				</div>

				<label for="input-file" class="btn btn-outline-secondary mt-3"
					id="image_container"> <span class="imgText">추가사진
						재업로드 (최대 10장)</span>
				...
				<div class="mt-5" id="image_container2">
					<c:if test="${not empty subImg }">
						<c:forEach var="subImg" items="${subImg}" varStatus="loop">
							<img src="<spring:url value='/boardImg/${subImg }'/>"
								class="board-file border border-secondary">
						</c:forEach>
					</c:if>
				</div>
				...
				<!-- 내용 입력란 -->
				<textarea class="form-control mt-5 " name="content"
					id="writeContent"><c:out value="${board.content}" /></textarea>
				<!-- 태그 리스트 나오는 곳 -->
				...
				<c:forEach var="tag" items="${tag}" varStatus="loop">
					<div style="display: inline;">
						#<input type="text" class="form-control col-auto" name="tag"
							value="<c:out value="${tag}" />"
							style="width: 100px; display: inline; margin: 0.5%;">
					</div>
				</c:forEach>
